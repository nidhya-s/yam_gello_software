...
class DynamixelDriver(DynamixelDriverProtocol):
    ...
    def _read_joint_states(self):
        ...
        fail_counts = {dxl_id: 0 for dxl_id in self._ids}
        while not self._stop_thread.is_set():
            ...
            try:
                with self._lock:
                    ...
                    for i, dxl_id in enumerate(self._ids):
                        ...
                        if self._groupSyncRead.isAvailable(
                            ...
                            fail_counts[dxl_id] += 1
                            if (
                                fail_counts[dxl_id] <= 5
                                or fail_counts[dxl_id] % 100 == 0
                            ):
                                print(
                                    f"[DXL] Servo ID {dxl_id} velocity unavailable "
                                    f"(fail #{fail_counts[dxl_id]})"
                                )
                            all_ok = False
                            continue
                        ...
                        if self._groupSyncRead.isAvailable(
                            ...
                            if (
                                ...
                                print(
                                    f"[DXL] Servo ID {dxl_id} position unavailable "
                                    f"(fail #{fail_counts[dxl_id]})"
                                )
                            all_ok = False
                            continue

                        fail_counts[dxl_id] = 0
                ...
                print(f"[DXL] Reading thread exception: {exc}")
                continue
    ...
    def close(self):
        ...
        if self._reading_thread.is_alive():
            print(f"[DXL] warning: reading thread did not stop for {self._port}")
